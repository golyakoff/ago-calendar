...
/// <summary>
/// The booking write, in one transaction: the person's thin operational record is upserted by
/// person id, the slot is claimed with a compare-and-set whose rows-affected count is the verdict,
/// and - for a booking with no chat origin - the locally minted person id is announced to the
/// account's person registry through the outbox (`adr/0184` decision 2).
///
...
///
/// <para><b>Why the person-record upsert is in the same transaction as the claim.</b> Not for
/// atomicity of the two writes as such - a record without a booking harms nothing operationally - but
/// because it is personal data. Writing a person's phone number for a booking that then loses the
...
/// no trace. It also gives the pair a single ordering rule: the person row is locked first, always,
/// so two bookings by the same person can never deadlock against each other. The
/// <c>PersonRegistered</c> outbox row rides the same transaction for the same reason, plus rule 4's:
/// a person id that was minted for a booking that never happened must not reach chat.</para>
///
...
/// <param name="Phone">The number this person is booking with - snapshotted onto the person record
/// (author decision O1 on `adr/0184`: the verified-phone gate and the per-phone rate limit need it
/// local). Personal data - see <c>personal-data.md</c>.</param>
/// <param name="RegisterPerson">
/// `adr/0184` decision 2: non-null exactly when <see cref="PersonId"/> was minted locally for this
/// attempt (no chat origin), carrying what the <c>PersonRegistered</c> event needs beyond the id - the
/// name the person typed, if any. Null for a chat-origin booking: chat already knows that person, and
/// announcing them again would be noise. The adapter stages the event onto the outbox inside the claim's
/// own transaction, so the announcement and the booking commit or roll back together.
/// </param>
...
/// reaches here with this genuinely null. When non-null, snapshotted onto the <c>person_records</c> row by
/// the adapter (kept, never overwritten, once first set for the same phone) so a person who verified once
/// for an earlier booking does not have to re-prove it for a later one from the same phone, matching
/// `20-09`'s own "Chat later unlinking the identity does not retroactively un-verify an already-claimed
/// booking" acceptance.
/// </param>
/// <param name="PersonId">
/// `26-136`/`adr/0184`: the account-scoped person this booking is for - the key of the person record
/// the adapter upserts, and the value stamped onto every claimed row's own <see cref="Event.PersonId"/>.
/// Non-nullable here on purpose - <c>BookEventHandler</c> mints one locally when the inbound request
/// carries none (`adr/0184` decision 2's public/operator path), so by the time an attempt reaches this
/// port a person id always exists, and making that a construction guarantee is stronger than a runtime
/// check any future caller of this port could forget.
/// </param>
...
public readonly record struct BookingAttempt(
    ...
    ServiceId ServiceId,
    PhoneNumber Phone,
    PersonRegistration? RegisterPerson,
    DateTimeOffset Now,
...
/// <summary>
/// `adr/0184` decision 2: what a locally minted person id is announced with, beyond the id itself -
/// see <see cref="BookingAttempt.RegisterPerson"/>. A struct of its own rather than a bare
/// <c>string?</c> on the attempt, so "register this person" and "the person typed no name" cannot be
/// confused: the first is the struct being present, the second is <see cref="DisplayName"/> being null.
/// </summary>
/// <param name="DisplayName">What the person typed as their name, or <see langword="null"/>. Blank is
/// normalised to null by the handler, never stored as an empty string.</param>
public readonly record struct PersonRegistration(string? DisplayName);
