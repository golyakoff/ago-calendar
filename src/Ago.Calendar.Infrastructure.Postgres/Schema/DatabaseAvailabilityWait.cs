...
/// <summary>
/// `20-21`: <b>the migrator waits for its database instead of losing a race to it.</b> Ported
/// unchanged in shape - the classification, the loop, and the reasoning - from
/// <c>Ago.Chat.Infrastructure.Postgres.Schema.DatabaseAvailabilityWait</c> (`8-10`).
///
/// <para><b>Why this exists here even though nothing has deployed AGO Calendar yet.</b> `8-10`'s own
/// incident (2026-08-26: a migrator Job started while Postgres was still restarting, failed, and left
/// a person to notice and re-apply it) has no analogue here to point at - but `20-20`'s own report
/// named this gap explicitly rather than pretending it away: "its init-container wait in `ago-deploy`
/// is therefore load-bearing, and nothing says so". `#312` is what turns that from a theoretical
/// coupling into a real one, which is why this closes before that item rather than after.</para>
///
/// <para><b>The distinction this type is built around</b> is unchanged from `8-10`: a genuine
/// <i>migration</i> failure must not be retried, so the wait wraps <b>only the connectivity probe</b>.
/// By the time <c>SchemaMigrationApplier</c> is constructed this class has already returned, so no
/// retry and no wait can reach a migration.</para>
///
/// <para><b>Why in-process rather than an init container.</b> The same reasoning the schema guard
/// records for itself (`ago-root#339`, a separate item): an init container cannot reach the
/// docker-compose loop or a bare <c>dotnet run</c>, which is where a developer meets this failure
/// first, and it would have to re-implement the classification below in shell - a <c>pg_isready</c>
/// loop cannot tell a wrong password from a slow start, which is precisely the mistake this type
...
public static class DatabaseAvailabilityWait
{
    ...
    /// one this item sets out to fix. Unknown means fail, always.</para>
    /// </summary>
    private static readonly HashSet<string> WaitableSqlStates = new(StringComparer.Ordinal)
    {
        ...
        // "...is in recovery mode". A Postgres mid-restart can accept a TCP connection and still
        // refuse to serve. This resolves by itself.
        "57P03",
        ...
        // the exact shape of the incident `8-10` closed.
        "57P01",
        "57P02",
    ...
    ///
    /// <para><c>ConnectionRefused</c> is the one the ago-chat incident produced - the Service exists,
    /// the pod behind it is restarting. The rest are the same condition seen from a different layer:
    /// DNS that has not caught up on a namespace applied from scratch, a SYN dropped while a node's
    /// iptables rules are in flux, a handshake cut short by a backend exiting.</para>
    ///
    ...
    /// <c>3D000</c> (the database does not exist), <c>42501</c> (insufficient privilege), <c>08P01</c>
    /// (protocol violation - whatever answered is not Postgres), and a malformed connection string.
    /// Each of those is permanent, each is a configuration mistake, and each would be turned into an
    /// unexplained ninety-second timeout by a more generous rule.</para>
    ///
    ...
    public static bool IsWorthWaitingFor(Exception? exception)
    {
        for (var current = exception; current is not null; current = current.InnerException)
        {
            switch (current)
            {
                ...
                // before saying anything a SQLSTATE could describe. `8-10` measured this against
                // Docker's own port proxy binding a published port before the Postgres behind it is
                // listening; the same shape recurs anywhere something terminates a connection ahead of
                // the backend. It is the same event as ConnectionReset above, seen as a clean EOF
                // rather than an RST.
    ...
    /// cannot fix.
    ///
    /// <para>Takes <paramref name="probe"/> as a delegate for the same reason the schema guard's own
    /// wait loop (`ago-root#339`) takes its inspection step as one: the interesting states - refused
    /// twice then accepted, refused until the clock runs out, rejected outright on the first attempt -
    /// are properties of this loop, and a real Postgres cannot be made to enter them on demand.</para>
    ///
