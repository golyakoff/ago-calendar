...
public class ContactsReportTests(PostgresFixture fixture)
{
    ...
    public async Task TheReadStore_IsTenantIsolated()
    {
        ...
        var extraOfMine = PersonRecord.Register(
            CalendarSeed.NewId(), mine.Tenant.Id, new PhoneNumber("+79998887766"), CalendarSeed.Now);

        await using (var db = fixture.CreateDbContext())
        {
            db.PersonRecords.Add(extraOfMine);
        ...
        Assert.Contains(rows, r => r.PersonId == extraOfMine.PersonId);
        Assert.DoesNotContain(rows, r => r.PersonId == theirs.Person.PersonId);
    }
    ...
    public async Task TheHandler_ReturnsEveryField_ForACallerHoldingCustomerRead()
    {
        ...
        person!.RecordVerifiedPhone(CalendarSeed.Now);
        await db.SaveChangesAsync();
        ...
        // `adr/0184`: the row is the calendar's own operational facts and the opaque person id - no name,
        // no notes; the console reads those from chat's own Person API through PersonId.
        var row = Assert.Single(result.Value);
        Assert.Equal(seed.Person.PersonId, row.PersonId);
        Assert.Equal(CalendarSeed.Now, row.PhoneVerifiedAt);
        Assert.Null(row.PhoneConfirmedByOperatorAt);
    ...
    public async Task TheConsoleEndpoint_ReturnsTheReport_OverRealHttp()
    {
        ...
        Assert.Equal(seed.Person.PersonId, Assert.Single(contacts!).PersonId);
    }
}
