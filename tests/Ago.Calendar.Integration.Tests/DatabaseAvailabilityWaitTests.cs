...
/// <summary>
/// `ago-root#340`: the wait-then-give-up loop, driven directly - ported unchanged in shape from
/// <c>Ago.Chat.Integration.Tests.DatabaseAvailabilityWaitTests</c> (`8-10`). The interesting states
/// (refused twice then accepted, refused until the clock runs out, rejected outright on the first
/// attempt) are properties of this loop, and a real Postgres cannot be made to enter them on demand.
///
/// <para>The end-to-end proof - a migrator process started against a port nothing is listening on,
/// which then waits and succeeds - is <see cref="SchemaMigratorTests.AMigratorStartedBeforePostgres_WaitsAndThenSucceeds"/>,
/// against a real container. These two files divide the work the way `8-10` divided its own: the loop
/// here, the deployable there.</para>
/// </summary>
public class DatabaseAvailabilityWaitTests
{
    ...
    public async Task WhenPostgresArrivesLate_ItWaitsAndThenSucceeds()
    {
        ...
        // Says so while it is happening, not only afterwards: an operator watching this run for ninety
    ...
    /// <summary>
    /// A wrong password must not be waited on: it is reported on the first attempt, so the operator
    /// reads "password authentication failed" rather than a timeout ninety seconds later.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Cancellation is not a classification. Ctrl-C or a SIGTERM propagates out rather than being
    /// reported as "gave up waiting" - the deployable was stopped, it did not fail, and saying
    /// otherwise would put a false infrastructure failure in the log of an ordinary shutdown.
    /// </summary>
    [Fact]
    ...
    /// class of drift `20-21`'s schema guard exists to prevent.
    /// </summary>
    [Fact]
