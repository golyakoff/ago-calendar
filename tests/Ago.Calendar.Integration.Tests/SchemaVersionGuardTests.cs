...
/// <summary>
/// `20-21` (`ago-root#339`): the wait-then-refuse loop, driven directly - ported unchanged in shape
/// from <c>Ago.Chat.Integration.Tests.SchemaVersionGuardTests</c> (`8-08`). No container and no
/// database - the behaviour worth pinning here is about the loop (does it re-check, does it give up,
/// does it give up at the right time), and driving that through a real migration would be slower and
/// prove less, because a real migration cannot be made to become current on the third poll on demand.
///
...
public class SchemaVersionGuardTests
{
    ...
    public async Task WhenTheSchemaNeverCatchesUp_ItThrowsAndNamesThePendingMigrations()
    {
        ...
        // The message is the deliverable as much as the exception type is - it names the migrator and
        // the pending migration rather than saying only "schema out of date".
        Assert.Contains("B", exception.Message, StringComparison.Ordinal);
        Assert.Contains("Ago.Calendar.Migrator", exception.Message, StringComparison.Ordinal);
    }
    ...
    /// window working as designed, and refusing here would make rollback impossible.
    /// </summary>
    [Fact]
